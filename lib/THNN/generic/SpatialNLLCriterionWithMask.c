...
#define TH_GENERIC_FILE "generic/SpatialNLLCriterionWithMask.c"
#else
          ...
          THTensor *total_weight)
{
  ...
  valid_mask = THTensor_(newContiguous)(valid_mask);
  weights = weights ? THTensor_(newContiguous)(weights) : NULL;

  real *input_data = THTensor_(data)(input);
  THIndex_t *target_data = THIndexTensor_(data)(target);
  real *valid_mask_data = THTensor_(data)(valid_mask);
  ...
  for (int b = 0; b < batch_size; b++) {
    for (int elem = 0; elem < map_size; elem++) {
      ...
      if (valid_mask_data[b * map_size + elem] < 1.0f)
        continue;
